...
The `Dockerfile` uses a two-stage build. The first stage compiles a release binary against `rust:1.94.1-bookworm`; the second stage copies it into a .NET SDK Debian base with common engine tools for Rust `1.94.1`, Go `1.25.3`, C/C++, Zig `0.15.2`, Java/Maven, JavaScript/npm, and Python/pip. Larger or version-sensitive SDKs such as Haskell and non-default Rust/.NET versions are left to custom images or mounted toolchains.
