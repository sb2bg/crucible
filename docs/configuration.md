...
admin_token = "change-me" # optional bearer token for /api/admin/*
...
| `admin_token`  | unset         | When set, the dashboard sends it as a `Bearer` token on every `/api/admin/*` request. |

The browser client stores the admin token in local storage. Clear the tab data, or use a fresh browser profile, if you want to reset it.
...
| `binary_path`           | -          | Path to the built binary, relative to the repository root.                                                         |
